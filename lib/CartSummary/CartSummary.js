  ...
  return (
    <div className={clsx(classNames?.root)}>
      ...
      {cartItems?.map((item) => (
        <div
          key={item.id}
          className={clsx(generateClasses(`grid grid-cols-[4fr,1fr,1fr] border-b border-gray-300 py-6`, classNames?.itemContainer))}
        >
          <div className="flex items-center">
            ...
            {!item.Image && item.imageUrl && (
              <img
                src={item.imageUrl}
                className={clsx(generateClasses('max-w-[100px] max-h-[100px] mr-5', classNames?.image)
                )}
              />
            )}
            <div>
              <p className={clsx(generateClasses('font-bold', classNames?.productTitle))}>
                {item.title}
              </p>
              <p
                className={clsx(generateClasses('text-sm text-gray-900 font-light', classNames?.variantTitle)
                )}
              >
                {item.variantTitle}
              </p>
          ...
          <div className="flex items-center">
            <div className="md:flex-grow px-2">
              {isReadOnly ? (
                <p
                  className={clsx(generateClasses('text-sm text-gray-500', classNames?.quantityLabel))}
                >
                  {item.quantity}
                </p>
              ) : (
          ...
          <div
            className={clsx(
              generateClasses('flex items-center justify-between', classNames?.pricingContainer),
            )}
          >
            <Pricing
            ...
            {!isReadOnly && (
              <button
                className={clsx(generateClasses('p-2', classNames?.removeButton))}
