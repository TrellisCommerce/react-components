      ...
      {cartItems?.map((item) => (
        ...
        >
          ...
          <div className="flex flex-col md:flex-row flex-grow">
            <div className="md:flex-grow px-2">
              ...
              {isReadOnly ? (
                ...
                <div className="flex items-center">
                  <label
                    htmlFor={`qty-${item.id}`}
                    className={clsx("text-sm text-gray-500 mr-1", classNames?.quantityLabel)}
                  >
                    {quantityLabel}
                  </label>
                  <fieldset className={clsx("flex border border-slate-300 w-min rounded-md overflow-hidden", classNames?.field?.root)}>
                    <input
                      id={`qty-${item.id}`}
                      type="text"
                      pattern="[0-9]*"
                      ...
                      min="1"
                      className={clsx("max-w-[2rem] m-1 text-center", classNames?.field?.input)}
                    />
                    <div className="flex flex-col border-l border-slate-300">
                      <button
                        className={clsx("border-b border-slate-300 px-1 text-slate-300 hover:text-black hover:bg-slate-100 focus:bg-slate-300 focus:text-black", classNames?.field?.plus)}
                        onClick={() => onChange(item.id, item.quantity + 1)}
                      >
                        +
                      </button>
                      <button
                        className={clsx("px-1 text-slate-300 hover:text-black hover:bg-slate-100 focus:bg-slate-300 focus:text-black", classNames?.field?.minus)}
                        onClick={() =>
                          item.quantity > 1 &&
            ...
            <div className={clsx("px-2 flex flex-col items-end", classNames?.pricingContainer)}>
              <Pricing
              ...
              {!isReadOnly && (
                <button
                  className={clsx("p-2 rounded-full hover:bg-slate-200 focus:bg-slate-300", classNames?.removeButton)}
                  onClick={() => onRemove(item.id)}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    className={clsx("fill-current text-red-600 w-4 h-4", classNames?.removeButtonIcon)}
                    labelledby="title"
                  >
