      ...
      {isColor ? (
        <>
          <h3
            className={clsx(generateClasses(
              'text-sm text-gray-900 font-medium', classNames.headingTypeLabel),
            )}
          >
            {typeLabel}
          </h3>
          ...
          >
            <RadioGroup.Label
              className={clsx(generateClasses('sr-only', classNames.radioGroupLabel))}
            >
              Choose a {typeLabel}
            </RadioGroup.Label>
            <div
              className={clsx( generateClasses('flex items-center space-x-3', classNames.radioGroupOptionsWrapper))}
            >
                ...
                >
                  {variant.isEnabled ? (
                    <>
                      <RadioGroup.Label
                        as="p"
                        className={clsx(generateClasses('sr-only', classNames.colorLabel))}
                      >
                        {typeLabel}
                      </RadioGroup.Label>
        ...
        <>
          ...
          <RadioGroup
            ...
            className={clsx(generateClasses('mt-6 max-w-xs', classNames.radioGroup))}
          >
            <RadioGroup.Label
              className={clsx(generateClasses('sr-only', classNames.radioGroupLabel))}
            >
                ...
                >
                  {({ active, checked }) => (
                    <>
                      ...
                      {variant.isEnabled ? (
                        <div
                          className={clsx(generateClasses('border absolute -inset-px pointer-events-none', classNames.radioGroupOptionItem),)}
                          aria-hidden="true"
                        />
                      ) : (
                        <div
