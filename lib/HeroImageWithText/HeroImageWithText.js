...
const HeroImageWithText = ({
  ...
  <section className={clsx([generateClasses('h-128', classNames.root)])}>
    <div className={clsx([generateClasses('h-full', classNames.container)])}>
      <div
        className={clsx([generateClasses('h-full md:flex md:items-stretch', classNames.layout)])}
      >
        <div
          className={clsx([generateClasses('md:mr-lg max-w-lg my-6 mx-8 md:my-12 md:mx-16 md:flex md:justify-center md:flex-col md:basis-full', classNames.textContainer)])}
        >
          <h4
            className={clsx([generateClasses(
              'mb-6 md:mb-sm font-black text-3xl', classNames.title)
            ])}
          >
            {titleText}
          </h4>
          <p className={clsx([generateClasses('text-lg', classNames.paragraph)])}>
            {paragraphText}
          </p>
        </div>
        <div
          className={clsx([generateClasses('w-full md:basis-full',
        ...
        >
          {Image ? (
            ...
            <div
              className={clsx([generateClasses('relative pb-3/4 md:pb-0 bg-primary overflow-hidden w-full md:h-full', classNames.imageWrapper)])}>
              <img
                className={clsx([generateClasses('top-0 w-full h-full min-h-2/3 object-cover absolute z-0', classNames.image)])}
                src={imageSrc}
                alt={imageAltText}
              />
            </div>
