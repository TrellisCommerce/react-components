...
const Metrics = ({ classNames, metrics }) => {
  return (
    <div className={clsx(['grid grid-cols-3 gap-4', classNames?.container])}>
      {metrics.map((metric) => (
        <div
          key={metric.id}
          className={clsx([
        ...
        >
          <div
            className={clsx(['text-6xl font-bold', classNames?.metricValue])}
          >
            {metric.value}
          </div>
          <div className={clsx(['pt-1', classNames?.metricLabel])}>
            {metric.label}
          </div>
        </div>
      ))}
