...
const Metrics = ({ classNames, metrics }) => {
  return (
    <div
      className={clsx([
    ...
    >
      {metrics.map((metric) => (
        <div
          key={metric.id}
          className={clsx([
            generateClasses('flex flex-col items-center', classNames?.metricWrapper)])}
        >
          <div
            className={clsx([generateClasses('text-6xl font-bold', classNames?.metricValue)])}
          >
            {metric.value}
          </div>
          <div className={clsx([generateClasses('pt-1', classNames?.metricLabel)])}>
            {metric.label}
          </div>
        </div>
      ))}
