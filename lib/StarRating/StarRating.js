  ...
  return (
    <>
      ...
      <div className={clsx(['flex', 'items-center'])}>
        <span className={clsx(['text-xs', 'mr-3'])}>
          {humanFriendlyStarRating}
        </span>
        <div className={clsx(['flex', 'items-center'])}>{renderStars()}</div>

        <span className={clsx(['text-xs', 'ml-6'])}>See all 453 reviews</span>
      </div>
    </>
  );
};
