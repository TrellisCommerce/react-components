...
const StarRating = ({ classNames, starRating, maximumStarCount = 5, StarSvgPaths = StarPath}) => {
    ...
    return (
        <>
            ...
            <div className={classnames(['flex', 'items-center'])}>
                <span className={classnames(['text-xs', 'mr-3'])}>{humanFriendlyStarRating}</span>
                <div className={classnames(['flex', 'items-center'])}>
                    {renderStars()}
                </div>

                <span className={classnames(['text-xs', 'ml-6'])}>See all 453 reviews</span>
            </div>
        </>
    );
}
